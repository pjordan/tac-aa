...
public class BarDiagram extends JComponent
{
  ...
  /**
   * Sets the names for the bars. The name and values will be shown as tooltips
   * if tooltips are turned on.
   * 
   * @param names
  ...
  private void setupColors()
  {
    if (leftColor != null && data != null)
    {
      ...
      for (int i = 0, n = data.length; i < n; i++)
      {
        allColors[i] = new Color((int) (r0 * (n - i) + r1 * i), (int) (g0 * (n - i) + g1 * i),
            (int) (b0 * (n - i) + b1 * i));
      }
    }
  }
    ...
    if (isShowingValue && g instanceof Graphics2D)
    {
      ...
      for (int i = 0, n = drawData.length; i < n; i++)
      {
        if (drawData[i] > 0)
        {
          ...
          g2d.drawString(Integer.toString(drawData[i]), py, x + i * xspace + 4 + xspace / 2);
        }
      }
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    bd.setData(new int[]
    { 12, 42, 12, 21, 55, 3, 15, 12, 42, 12, 21, 55, 3, 15 });
    ...
    {
      bd.setData(new int[]
      { 12, 42, 12, 21, 55, 3, 15, 12, 42, 12, 21, 55, 13, 15 });
      Thread.sleep(500);
      bd.setData(new int[]
      { 12, 24, 21, 12, 12, 21, 55, 3, 15, 32, 55, 23, 33, 15 });
      Thread.sleep(500);
    }
  }
