...
public class DoubleField extends FileDBField
{
  ...
  protected DoubleField(FileDBTable table, String name, int type, int size, int flags,
      Object defaultValue)
  {
  ...
  double getDouble(int index)
  {
    ...
    {
      throw new IllegalArgumentException("no such object: " + index + ",size="
          + table.getObjectCount());
    }
    return values[index];
  }
  ...
  {
    System.arraycopy(values, index + 1, values, index, table.getObjectCount() - index - 1);
  }
  ...
  protected void prepareSet(int index, Object value)
  {
    ...
    if (isUnique()
        && (indexOf(lastValue, 0, index) >= 0 || indexOf(lastValue, index + 1, table
            .getObjectCount()) >= 0))
    {
      throw new IllegalArgumentException("An object with " + name + " = '" + lastValue
          + "' already exists");
    }
    lastIndex = index;
  }
