...
public class BinaryTransportReader extends TransportReader implements BinaryTransport
{
  ...
  private boolean skipToNextNode() throws ParseException
  {
    ...
    else
    {
      ...
      // \TODO UNLESS we are in "enter node" mode, then we just need to find
  ...
  private int skipNode(int nodePos, int valPos)
  {
    ...
    for (int i = 0; i < attNo; i++)
    {
      switch (messageData[nodePos] & 0xff)
      {
        ...
        case STRING:
        {
          int slen = ((messageData[valPos] & 0xff) << 8) + (messageData[valPos + 1] & 0xff);
          valPos += slen + 2;
          break;
        }
  ...
  {
    return ((messageData[pos] & 0xff) << 24) + ((messageData[pos + 1] & 0xff) << 16)
        + ((messageData[pos + 2] & 0xff) << 8) + (messageData[pos + 3] & 0xff);
  }
  
  private long getLValue(int pos)
  {
    long v = ((messageData[pos] & 0xffL) << 56) + ((messageData[pos + 1] & 0xffL) << 48)
        + ((messageData[pos + 2] & 0xffL) << 40) + ((messageData[pos + 3] & 0xffL) << 32)
        + ((messageData[pos + 4] & 0xffL) << 24) + ((messageData[pos + 5] & 0xffL) << 16)
        + ((messageData[pos + 6] & 0xffL) << 8) + (messageData[pos + 7] & 0xffL);
    return v;
  }
    ...
    while (pos < end)
    {
      ...
      switch (c >> 4)
      {
        ...
        case 14:
          ...
          buf[index++] = (char) (((c & 0x0F) << 12) | ((char2 & 0x3F) << 6) | ((char3 & 0x3F) << 0));
          break;
        default:
          /* 10xx xxxx, 1111 xxxx */
          throw new ParseException("malformed UTF-8", pos);
  ...
  public boolean nextNode(String name, boolean isRequired) throws ParseException
  {
    ...
    if (isRequired)
    {
      throw new ParseException("node '" + name + "' not found", currentPosition);
    }
    return false;
  }
  ...
  private Object getAttributeAsObject(int id) throws ParseException
  {
    ...
    for (int i = 0; i < attNo; i++)
    {
      int nid = ((messageData[pos + 1] & 0xff) << 8) + (messageData[pos + 2] & 0xff);
      switch (messageData[pos] & 0xff)
      {
        case INT:
          if (nid == id)
  ...
  private String getAttributeAsString(int id) throws ParseException
  {
    ...
    {
      throw new ParseException("Illegal object type: " + aVal, currentPosition);
    }
  }
  
  protected String getAttribute(String name, String defaultValue, boolean isRequired)
      throws ParseException
  {
    ...
    if (value == null)
    {
      if (isRequired)
      {
        throw new ParseException("attribute " + name + " not found", currentPosition);
      }
      return defaultValue;
    }
    else
  ...
  protected int[] getAttributeAsIntArray(String name, boolean isRequired) throws ParseException
  {
    ...
    if (val != null)
    {
      ...
      {
        throw new ParseException("Illegal value type, expected int[] got " + val, 0);
      }
    }
    return null;
  }
