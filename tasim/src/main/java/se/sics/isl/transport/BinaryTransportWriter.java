...
public class BinaryTransportWriter extends TransportWriter implements BinaryTransport
{
  ...
  public void write(byte[] buffer)
  {
    ...
    {
      throw new IndexOutOfBoundsException("Too many bytes to fit array, " + "requires "
          + (aliasSize + nrOfBytes) + ", got " + buffer.length);
    }
    if (aliasSize > 0)
    {
  ...
  private void writeCurrentNode(byte nodeType, int pos)
  {
    ...
    else
    {
      ...
      {
        int other = 1 - pos;
        if (isTablesSupported && nodeName[other] == name && nodeWritten[other]
  ...
  private void writeString(String value)
  {
    ...
    {
      byteData = (byte[]) ArrayUtils.setSize(byteData, nrOfBytes + maxSize + DATA_SIZE);
    }
  ...
  private void writeAlias(int id, String name)
  {
    ...
    {
      aliasData = (byte[]) ArrayUtils.setSize(aliasData, aliasSize + maxSize + ALIAS_SIZE);
    }
  ...
  private void writeIntArr(int[] value)
  {
    ...
    {
      byteData = (byte[]) ArrayUtils.setSize(byteData, nrOfBytes + len * 4 + DATA_SIZE);
    }
    writeShort(len);
