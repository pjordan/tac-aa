...
final class Admin
{
   ...
   * is allowed (after a simulation has been locked it can no longer be removed)
   */
      ...
      IOException
  {
    ...
    this.runAfterSimulation = config.getProperty(CONF + "runAfterSimulation");
    ...
    this.allowUnregisteredAgents = config.getPropertyAsBoolean(CONF + "allowUnregisteredAgents", false);
    
    //  MLB 20080517 - units of milliseconds
    this.gameStartOffset = config.getPropertyAsInt(CONF + "gameStartOffset", 0) * 1000;
    ...
    //  added by WG20071030 
    this.startsimid = config.getPropertyAsInt(CONF + "sim.startsimid",config.getPropertyAsInt("sim.startsimid",0)); 
    this.maxgames = config.getPropertyAsInt(CONF + "sim.gamestorun",config.getPropertyAsInt("sim.gamestorun",-1)); 
    ...
    for (int i = 0, n = serverName.length(); i < n; i++)
    {
      ...
      {
        throw new IllegalConfigurationException("server name may only contain "
    ...
    try
    {
      infoConnection = (InfoConnection) Class.forName(infoClass).newInstance();
    }
    catch (Exception e)
    {
      throw (IllegalConfigurationException) new IllegalConfigurationException(
          "could not create info " + "connection of type " + infoClass)
          .initCause(e);
    }
    ...
    // The simulation list should be checked once per minute on the minute, but 
    //  potentially offset by this.gameStartOffset (MLB 20080517)
    long currentTime = System.currentTimeMillis();
    long nextTime = (currentTime / 60000) * 60000 + 60000 + this.gameStartOffset; //MLB 20080517
      ...
      String simTypePattern)
  {
    for (int i = 0, n = simQueue.size(); i < n; i++)
    {
      ...
      {
        // This simulation has not yet received a simulation id which means
        // that no further simulation is initialized.
        break;
      }
      
      if (simulation.isParticipant(userID)
          && ((simTypePattern == null) || matchSimulationType(simTypePattern,
              simulation.getType())))
      {
        return simulation;
      }
      ...
      String simulationType)
  {
    ...
    else if (simTypePattern.charAt(len - 1) == '*')
    {
      return simulationType != null
          && simulationType.regionMatches(0, simTypePattern, 0, len - 1);
    }
    else
    {
   ...
   * the simulation where the agent joined, creating new simulations as needed.
   * Returns information about the joined simulations.
   * 
   ...
   */
  public synchronized SimulationInfo joinSimulation(int userID, String simType,
      String simParams, String simRoleName) throws NoSuchManagerException
  {
    if (simType == null)
    {
    ...
    for (int i = 0, n = simQueue.size(); i < n; i++)
    {
      ...
      else if ((startTime - time) > simulationLength)
      {
        ...
        {
          // Could not add to newly created means that we can never create
          // such a simulation
          return null;
        }
  ...
  /**
   * Notify the information server about a new participant in a simulation. Also
   * called by the simulation when adding dummy agents.
   * 
   * @param info
   *          the simulation that was joined
   * @param userID
  ...
  private void simulationStarted(Simulation simulation)
  {
    ...
    if (viewerConnection != null)
    {
      viewerConnection.simulationStarted(info.getSimulationID(),
          info.getType(), info.getStartTime(), info.getEndTime(), timeUnitName,
          timeUnitCount);
    }
    infoConnection.simulationStarted(info.getID(), timeUnitName, timeUnitCount);
  }
  ...
  void sendStateToInfoSystem()
  {
    ...
    if (simQueue.size() > 0)
    {
      ...
      synchronized (this)
      {
        sims = (SimulationInfo[]) simQueue.toArray(new SimulationInfo[simQueue
            .size()]);
      }
      if (sims != null)
      {
  ...
  void resultsGenerated(int simulationID)
  {
    ...
    if (!sourceFile.renameTo(new File(targetName)))
    {
      ...
      log.warning("could not use simple rename to move simulation log "
          + simulationID + " from " + simFileName + " to " + targetName);
      try
      {
    ...
    if (runAfterSimulation != null)
    {
      ...
      new Thread(new Runnable() {
        public void run()
        {
          try
          {
            String command = format(runAfterSimulation, "sg", new String[]
            { serverName, simulationIDAsString });
            if (command != null)
            {
          ...
          {
            log.log(Level.SEVERE, "could not run '" + runAfterSimulation
                + "' after simulation " + simulationIDAsString, e);
          }
        }
      ...
      boolean notifyInfoServer)
  {
    ...
    try
    {
      ...
      if (startIndex < 0)
      {
        ...
        if (notifyInfoServer)
        {
          infoConnection.requestFailed(InfoConnection.SCHEDULE_COMPETITION,
              competitionID, "conflicting locked game");
        }
      }
      else if (count == 0)
      {
        ...
        if (notifyInfoServer)
        {
          infoConnection.requestSuccessful(InfoConnection.SCHEDULE_COMPETITION,
              competitionID);
        }
      ...
      else
      {
        ...
        // Create all simulations first to give them a continuous id span
        SimulationManager manager = getSimulationManager(simType);
        ...
        for (int i = 0; i < count; i++)
        {
          ...
          SimulationInfo info = createSimulationInfo(manager, simType,
              simParams);
          simulations[i] = info;
          if (participants != null)
          {
            ...
            for (int j = 0, m = participants.length; j < m; j++)
            {
              ...
              {
                throw new IllegalStateException("could not join participant "
                    + participants[j] + " as " + role + " in simulation " + i
        ...
        for (int i = 0; i < count; i++)
        {
          ...
          if (endTime > nextReservationTime)
          {
            ...
            nextReservationTime = (reserveIndex < schedule
                ...
                .getReservationStartTime(reserveIndex) : Long.MAX_VALUE;
          ...
          {
            SimulationInfo reservedInfo = createTimeReservation(nextTime,
                reservationLength);
            simQueue.add(startIndex++, reservedInfo);
            simulationCreated(reservedInfo);
            nextTime = roundTimeToMinute(reservedInfo.getEndTime()
            ...
            // Reset the games counter to 1 since we will add a game below
            gamesCounter = 1;
          }
          ...
          while (endTime > nextExisting)
          {
            ...
            simulationRemoved((SimulationInfo) simQueue.remove(startIndex),
                "by competition");
    ...
    {
      log.log(Level.SEVERE, "could not schedule competition " + competitionID,
          e);
      if (notifyInfoServer)
      {
        infoConnection.requestFailed(InfoConnection.SCHEDULE_COMPETITION,
  ...
  synchronized void lockNextSimulations(int simulationCount)
  {
    ...
    {
      SimulationInfo simulation = (SimulationInfo) simQueue.get(i);
      if (!simulation.hasSimulationID() && !assignSimulationIDs(simulation))
      {
      ...
      boolean notifyInfoServer)
  {
    ...
    for (; index >= 0; index--)
    {
      ...
      else if (simulation.getStartTime() < endTime)
      {
        ...
        if (notifyInfoServer)
        {
          infoConnection.requestFailed(InfoConnection.RESERVE_TIME, 0,
              "conflicting simulation " + simulation.getID());
          startTime = -1L;
        }
      }
    }
      ...
      boolean notifyInfoServer)
  {
    ...
    try
    {
      ...
      SimulationManager manager = getSimulationManager(simType);
      SimulationInfo newSimulation = createSimulationInfo(manager, simType,
          simParams);
      int simulationLength = newSimulation.getSimulationLength() + SIM_DELAY;
      int index = 0;
      long time = ((getServerTime() + startDelay + 59000) / 60000) * 60000;
      if (time < getServerTime() + 2000)
      {
      ...
      if (notifyInfoServer)
      {
        infoConnection.requestSuccessful(InfoConnection.CREATE_SIMULATION,
            newSimulation.getID());
      }
      return true;
    }
    ...
    {
      log.log(Level.SEVERE, "could not create simulation of type '" + simType
          + "' (" + simParams + ')', e);
      if (notifyInfoServer)
      {
        infoConnection.requestFailed(InfoConnection.CREATE_SIMULATION, 0, e
            .getMessage());
      }
      return false;
    }
  ...
  synchronized void removeSimulation(int simulationUniqID)
  {
    for (int i = simQueue.size() - 1; i >= 0; i--)
    {
      ...
      {
        // No futher simulations can be removed because they have been locked
        break;
      }
    ...
    if (info != null)
    {
      ...
      if (currentTime >= startTime)
      {
        ...
        if (simulation != null)
        {
          ...
          {
            // End time already passed but the simulation has not ended itself.
            log.warning("requesting overdue simulation "
                + simulation.getSimulationInfo().getSimulationID() + " to end");
        ...
        else if (!info.hasSimulationID() && !assignSimulationIDs(info))
        {
          // It was not possible to assign a simulation id so the simulation
          // should be scratched
        ...
        {
          // The game is already at least thirty seconds late. It must be
          // scratched because simulations must not be started this
          // late.
          log.severe("scratching simulation " + info.getSimulationID()
        ...
        else
        {
          ...
          try
          {
            SimulationManager manager = getSimulationManager(info.getType());
            String simLogFile = SIM_FILE_PREFIX + info.getSimulationID()
                + SIM_FILE_SUFFIX;
            int inError = 0;
            ...
            try
            {
              simulation.init(this, info, simLogFile, simulationThreadPool);
              simulation.setup();
              inError = 1;
  ...
  final int loginAgentChannel(AgentChannel agent, String password)
  {
    ...
    if (userID == -1)
    {
      ...
      if (userID == -1)
      {
        ...
        {
          userID = infoConnection.addUser(name, password, "autocreated");
        }
  ...
  final void agentChannelAvailable(AgentChannel agent)
  {
    ...
    synchronized (this)
    {
      ...
      else
      {
        ...
        {
          channelList = (AgentChannel[]) ArrayUtils.setSize(channelList,
              channelNumber + 8);
        }
  ...
  {
    AgentChannel agent = (AgentChannel) channelTable.get(message.getReceiver());
    if (agent != null)
    {
      ...
      AdminContent content)
  {
    ...
    try
    {
      SimulationInfo info = joinSimulation(userID, simType, simParams, simRole);
      if (info != null)
      {
        String role = getSimulationRoleName(info, userID);
    ...
    catch (Exception e)
    {
      ...
      return new AdminContent(AdminContent.JOIN_SIMULATION,
          AdminContent.NO_SIMULATION_CREATED, e.getMessage()); // Better error
      // message. FIX
      // THIS!!!
    }
  }
