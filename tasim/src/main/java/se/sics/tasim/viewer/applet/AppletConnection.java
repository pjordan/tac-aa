...
public class AppletConnection implements Runnable {
  ...
  private static final Logger log =
    Logger.getLogger(AppletConnection.class.getName());
  ...
  public boolean connect() {
    try {
      ...
      String statusMessage = "Connecting to server " + serverName + " at "
	+ serverHost + ':' + serverPort;
      ...
      transportWriter.node("auth")
	.attr("serverName", serverName)
	.attr("userName", userName)
	.attr("version", ViewerApplet.VERSION)
	.endNode("auth");
  ...
  public void run() {
	...
	while (!finish && !connect()) {
	  applet.setStatusMessage("Failed to connect to "
				  + applet.getServerName() + " at "
				  + serverHost + " (will retry)");
	  Thread.sleep(30000);
	}
  ...
  private void parseMessage(byte[] buffer, int offset, int size) {
	...
	if (reader.isNode("intUpdated")) {
	  viewer.dataUpdated(reader.getAttributeAsInt("agent"),
			     ...
			     reader.getAttributeAsInt("value"));

	} else if (reader.isNode("longUpdated")) {
	  viewer.dataUpdated(reader.getAttributeAsInt("agent"),
			     reader.getAttributeAsInt("type", 0),
			     reader.getAttributeAsLong("value"));
	  ...
	  viewer.interaction(reader.getAttributeAsInt("fromAgent"),
			     reader.getAttributeAsInt("toAgent"),
			     reader.getAttributeAsInt("type", 0));

	} else if (reader.isNode("interactionWithRole")) {
	  viewer.interactionWithRole(reader.getAttributeAsInt("fromAgent"),
	  ...
	  viewer
	    .simulationStarted(reader.getAttributeAsInt("id"),
			       reader.getAttribute("type"),
			       reader.getAttributeAsLong("startTime"),
			       reader.getAttributeAsLong("endTime"),
			       reader.getAttribute("timeUnitName", null),
			       reader.getAttributeAsInt("timeUnitCount", 0));

	} else if (reader.isNode("participant")) {
	  viewer.participant(reader.getAttributeAsInt("id"),
			     reader.getAttributeAsInt("role"),
			     reader.getAttribute("name"),
			     reader.getAttributeAsInt("participantID"));
	  ...
	  viewer.intCache(reader.getAttributeAsInt("agent"),
			  reader.getAttributeAsInt("type", 0),
			  reader.getAttributeAsIntArray("cache"));
	} else {
