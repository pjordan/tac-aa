...
public class RandomManufacturer extends Agent
{
  ...
  public RandomManufacturer()
  {}
  
  private void checkInitialized()
  {
    boolean oldInitialized = this.isInitialized;
    this.isInitialized = this.bomBundle != null && this.catalog != null && this.factory != null;
    
    if (oldInitialized != this.isInitialized && this.isInitialized)
    {
      log.info(getName() + " requesting setup inventory!!!");
      for (int i = 0, n = catalog.size(); i < n; i++)
      {
        int productID = catalog.getProductID(i);
        String[] suppliers = catalog.getSuppliers(i);
        if (suppliers != null)
        {
          for (int j = 0, m = suppliers.length; j < m; j++)
          {
            RFQBundle b = (RFQBundle) supplyTable.get(suppliers[j]);
            if (b == null)
            {
              b = new RFQBundle();
              supplyTable.put(suppliers[j], b);
            }
            
            // Take one supplier by random
            int rfqID = ++rfqCounter;
            b.addRFQ(rfqID, productID, 10000, 0, currentDate + 2, 0);
          }
        }
      }
  ...
  protected void simulationSetup()
  {
    ...
    // will be created.
    this.log = Logger.getLogger(RandomManufacturer.class.getName() + '.' + getName());
  ...
  protected void messageReceived(Message message)
  {
    ...
    else if (content instanceof OfferBundle)
    {
      ...
      {
        if (random.nextDouble() >= 0.5 || currentDate < 3)
        {
      ...
      // int basePrice = bomProductIndex >= 0
      // ? bomBundle.getProductBasePrice(bomProductIndex)
      // : 0;
      ...
      // int offerID = addCustomerOffer(sender, rfq.getRFQID(i),
      // productID, quantity, dueDate);
      ...
      // }
      // int rfqID = ++rfqCounter;
      // // Trick: specify a too early due date in order to only
      // // receive the earliest complete offer (does no longer
      // // need to handle any partial offers ;)
    ...
    else if (content instanceof SimulationStatus)
    {
      ...
      // int rfqNumber = random.nextInt(5);
      // for (int r = 0; r < rfqNumber; r++) {
      // // Take one supplier by random
      // int rfqID = ++rfqCounter;
      // b.addRFQ(rfqID, productID, random.nextInt(2000),
      // currentDate
      // + (lastBidDueDate > currentDate
      // ? random.nextInt(lastBidDueDate - currentDate)
      // : 0));
      // }
      // }
      // }
    ...
    else if (content instanceof StartInfo)
    {
      ...
      this.numberOfDays = info.getNumberOfDays();
