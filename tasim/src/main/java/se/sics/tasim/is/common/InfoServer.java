...
public class InfoServer
{
  ...
  private static final Logger log = Logger
      .getLogger(InfoServer.class.getName());
  ...
  public InfoServer(ConfigManager config) throws IllegalConfigurationException,
      IOException
  {
    this.config = config;
    ...
    // specified in the configuration or if creation failed). FIX THIS!!! TODO
    ...
    basePath = config.getProperty(CONF + "resultDirectory", "./public_html/");
    if (basePath.length() > 0)
    {
      ...
      {
        throw new IllegalConfigurationException("simulation result directory '"
            + basePath + "' does not exist or is not " + "a directory");
      }
    }
    
    this.database = DatabaseUtils.createDatabase(config, CONF);
    this.userDatabase = DatabaseUtils
        .createUserDatabase(config, CONF, database);
    this.storeResults = config.getPropertyAsBoolean(CONF + "database.results",
        false);
    ...
    if (userTable == null)
    {
      ...
      userTable.createField("parent", DBField.INTEGER, 32, 0, new Integer(-1));
      userTable
          .createField("name", DBField.STRING, MAX_USER_PASSWORD_LENGTH, 0);
      userTable.createField("password", DBField.STRING,
          MAX_USER_PASSWORD_LENGTH, 0);
      userTable.createField("email", DBField.STRING, MAX_USER_EMAIL_LENGTH, 0);
    ...
    else
    {
      ...
      {
        userTable
            .createField("parent", DBField.INTEGER, 32, 0, new Integer(-1));
        userTable.flush();
      }
    ...
    // Shortcuts to the first SCM server for this info server. FIX THIS!!! TODO
    ...
    if (registrationURL == null
        || !config.getPropertyAsBoolean(CONF + "registration.disabled", false))
    {
      String notification = config.getProperty(CONF
          + "registration.notification");
      String password = config.getProperty(CONF + "registration.password");
      boolean isRemoteRegistrationEnabled = config.getPropertyAsBoolean(CONF
          + "registration.remote", false);
  ...
  {
    // Should check server name, password and SimConnection. FIX THIS!!! TODO
    ...
    else
    {
      ...
      Database serverDatabase = DatabaseUtils.createChildDatabase(config, CONF,
          serverName, database);
      SimServer server = new SimServer(this, serverDatabase, connection, path,
          storeResults);
      boolean firstServer = servers == null;
      servers = (SimServer[]) ArrayUtils.add(SimServer.class, servers, server);
      menuPage.setPage(getMenuData());
      statusPage.setPage(getStatusData());
      if (firstServer)
      {
        // Redirect all shortcuts to the first registered server. FIX THIS!!!
        // TODO
  ...
  public void validateUserInfo(String name, String password, String email)
  {
    ...
    for (int i = 0, n = name.length(); i < n; i++)
    {
      ...
      if (c <= 32)
      {
        throw new IllegalArgumentException("name may only contain characters "
            + "or digits");
      }
    }
  ...
  {
    // Validate that all information is correct and no conflicting agent exists
  ...
  public synchronized int claimUser(String name, String password, String email)
  {
    ...
    if (pwd == null)
    {
      ...
      throw new IllegalStateException("could not find password for agent "
          + name);
    }
  ...
  private String getMenuData()
  {
    ...
    if (servers != null)
    {
      for (int i = 0, n = servers.length; i < n; i++)
      {
        ...
        link(page, '/' + name + "/games/", "Coming games (watch, create)");
        link(page, '/' + name + "/history/", "Game History");
        // link(page, '/' + name + "/scores/", "Score Table"); FIX THIS!!! TODO
      }
    }
    page.close();
    return page.toString();
  ...
  private void title(HtmlWriter page, String title, boolean whitespace)
  {
    ...
    page.tr().td().attr("bgcolor='#202080'").tag("font",
        "face='Arial,Helvetica,sans-serif' color=white").tag('b').text(title)
        .tagEnd('b').tagEnd("font");
  }
  
  private void link(HtmlWriter page, String url, String text)
  {
    page.tr().td().tag('a').attr("href", url).attr("target=content").tag(
        "font", "face='Arial,Helvetica,sans-serif'").text(text).tagEnd("font")
        .tagEnd('a');
  }
