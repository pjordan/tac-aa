...
public class ComingPage extends HttpPage
{
  ...
  private static final Logger log = Logger
      .getLogger(ComingPage.class.getName());
    ...
    if (HttpRequest.__POST.equals(request.getMethod()))
    {
      ...
      if (request.getParameter("createsim") != null)
      {
        ...
        if (agentNoStr != null)
        {
          try
          {
            agentID = Integer.parseInt(request.getParameter("agent_no"))
                + userID;
          }
          catch (Exception e)
          {
        ...
        while (nameIterator.hasNext())
        {
          ...
          if (name.startsWith("jg_"))
          {
            if (simServer.isWebJoinActive())
            {
              ...
              try
              {
                int uniqSimID = Integer.parseInt(name.substring(3));
                int maxScheduled = simServer.getMaxAgentScheduled();
                int scheduledCount;
                agentID = Integer.parseInt(request.getParameter("agent_no"))
                    + userID;
                ...
                {
                  message = "Agent " + simServer.getUserName(sim, agentID)
                ...
                {
                  Event event = new Event(sim.getID(), agentID);
                  addEvent(event);
                  // Add support for simulation roles. FIX THIS!!! TODO
                  simServer.joinSimulation(uniqSimID, agentID, null);
                  message = event.waitForResult();
                  if (message == null)
                  {
                    removeEvent(event);
                    message = "Failed to join game " + getSimIDAsString(sim);
                  }
                }
              }
              catch (Exception e)
              {
                message = "Could not join game " + getSimIDAsString(sim) + ": "
                    + e.getMessage();
  ...
  private String getSimIDAsString(SimulationInfo info)
  {
    ...
    return gid <= 0 ? "starting at "
        + infoServer.getServerTimeAsString(info.getStartTime()) : Integer
        .toString(gid);
  }
      ...
      throws HttpException, IOException
  {
    ...
    else
    {
      ...
      if (currentComp != null)
      {
        ...
        {
          page.text(" (game ").text(currentComp.getStartSimulationID()).text(
              " - ").text(currentComp.getEndSimulationID()).text(')');
        }
        page.text("</h3>\r\n").text("<em>").text("Competition started at ")
            .text(infoServer.getServerTimeAsString(currentComp.getStartTime()))
            .text(" and ends at ").text(
                infoServer.getServerTimeAsString(currentComp.getEndTime()))
            .text(".</em>\r\n");
      ...
      else
      {
        ...
        if (nextCompetition != null)
        {
          nextCompetitionStarts = infoServer
              .getServerTimeAsString(nextCompetition.getStartTime());
          page.text("<b>Next competition '").text(nextCompetition.getName())
              .text("' begins at ").text(nextCompetitionStarts).text(
                  "</b><p>\r\n");
      ...
      if (allowJoin)
      {
        page.text(
            "Select agent for joining: "
                + "<select name=agent_no><option value=0>").text(userName);
        int id = agentID;
        if (id > 0)
        {
          id = id - userID;
      ...
      page.text("\r\n<table border=1><tr><th>ID</th><th>Time</th>"
          + "<th>Type</th>"
          + "<th>Participants</th><th>Status</th><th>Join</th>" + "</tr\r\n>");
      ...
      for (int i = 0, n = numberOfSimulations; i < n; i++)
      {
          ...
          if (startSimulation == uniqSimulationID && nextCompetition != null)
          {
            page.text(
                "<tr><td bgcolor='#e0e0ff' colspan=6>&nbsp;</td></tr>"
                    ...
                    + "<font size=+1 color='#800000'><b>Competition ").text(
                nextCompetition.getName()).text(" begins");
        ...
        appendTimeMillis(page, simulation.getEndTime()).text("</td>").text(
            columnStart);
        page.text(simServer.getSimulationTypeName(simulation.getType())).text(
            "</td>").text(columnStart);
        for (int p = 0, np = simulation.getParticipantCount(); p < np; p++)
        {
          ...
          if (participant >= minAgentID && participant <= maxAgentID)
          {
            page.text("<font size=+1 color='#800000'><b>").text(
                simServer.getUserName(simulation, participant)).text(
                "</b></font>");
          }
          else if (participant < 0)
          {
            page.tag("em").text(simServer.getUserName(simulation, participant))
                .tagEnd("em");
          }
          else
          {
            page.text(simServer.getUserName(simulation, participant));
          }
        }
        page.text("&nbsp;</td>").text(columnStart).text(
            isRunning ? "Running" : "Coming").text("</td>").text(columnStart);
        ...
        {
          page.text("<input type=submit value='Join' name='jg_").text(
              uniqSimulationID).text("'>");
        }
        page.text("</td></tr\r\n>");
        if (endSimulation == uniqSimulationID)
        {
          page.text("<tr><td bgcolor='#e0e0ff' colspan=6>&nbsp;</td></tr>"
              + "<tr><td colspan=6 align=center>"
      ...
      {
        page.text(
            "<br><em>(Only showing the first " + MAX_VISIBLE_SIMULATIONS
                + " of the coming ").text(simulationsLen).text(" games)</em>");
      }
    }
    ...
    if (allowJoin && (simulationsLen <= MAX_VISIBLE_SIMULATIONS))
    {
      ...
      else
      {
        ...
        for (int i = 0, n = simTypes.length; i < n; i++)
        {
          ...
          page.text('>').text(simServer.getSimulationTypeName(type)).text(
              "</option>\r\n");
        }
        page.text("</select>\r\n");
      }
