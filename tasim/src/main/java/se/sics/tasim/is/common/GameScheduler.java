...
public class GameScheduler extends HttpPage
{
  ...
  private void handlePreview(HttpRequest req, StringBuffer page)
  {
    ...
    try
    {
      ...
      if (parentIDStr != null)
      {
        ...
        page.append("<p><font face='arial' size='+1'>Agents in parent "
            + "competition " + parentCompetition.getName() + "</font>"
            + "<p>\r\n");
      ...
      page.append(
          "<font face='arial' size='+1'>Competition Data</font>"
              + "<p>\r\n<table border='0'>"
              + "<tr><td>Competition name:</td><td>").append(name);
      ...
      if (scoreGenerator != null)
      {
        page.append(
            "</td></tr\r\n>"
                + "<tr><td>Competition score table generator</td><td>").append(
            scoreGenerator);
      }
      page.append(
          "</td></tr\r\n>" + "<tr><td>Total number of players:</td><td>")
          .append(nrUsers).append(
      ...
      page.append(
          ...
          .append(rounds * minGames).append(
              "</td></tr\r\n>" + "<tr><td>Number of games per round:</td><td>")
          .append(minGames).append(
              "</td></tr>" + "<tr><td>Number of rounds:</td><td>" + rounds
                  + "</td></tr\r\n>"
                  + "<tr><td>Number of games per agent/round:</td><td>"
                  + perAgent + "</td></tr\r\n>"
                  + "<tr><td>Number of games per agent:</td><td>").append(
              perAgent * rounds).append("</td></tr\r\n>"
          // + "<tr><td>Game Length:</td><td>")
              // .append(gameLength)
              // .append(" min</td></tr\r\n>"
              + "<tr><td>Start Time:</td><td>").append(
              formatServerTimeDate(time)).append(
              "</td></tr\r\n>"
                  + "<tr><td>Approx End Time (55 min games):</td><td>");
      long endTime = time + (rounds * minGames) * (55 + timeBetween) * 60000L;
      ...
      page.append(
          "</table>\r\n<p>\r\n" + "<form method=post>"
              + "<input type=hidden name=agentNo value=").append(nrUsers)
          .append("><input type=hidden name=rounds value=").append(rounds)
          .append('>');
      ...
      page.append("<input type=hidden name=reserveTime value=").append(
          reserveTime).append('>');
      page.append("<input type=hidden name=reserveBetween value=").append(
          reserveBetween).append('>');
    ...
    catch (Exception e)
    {
      ...
      page.append("Could not schedule games: <font color=red>").append(e)
  ...
  private void handleExecute(HttpRequest req, StringBuffer page)
  {
    ...
    try
    {
      ...
      else
      {
        ...
        if (noWeights)
        {
          schedule.setFlags(schedule.getFlags() | Competition.NO_WEIGHT);
        }
        if (startWeightDuringWeekends)
        {
          schedule.setFlags(schedule.getFlags() | Competition.WEEKEND_LOW);
        }
        if (lowestScoreAsZero)
        {
        ...
        page.append("Requested ").append(scheduledGames.length).append(
            " scheduled games in competition ").append(name).append(".<p>");
      }
    }
    catch (Exception e)
    {
      log.log(Level.WARNING, "could not create competition", e);
      page.append("Competition could not be created: <font color=red>").append(
          e).append("</font>");
    }
  }
  ...
  private void handleConfiguration(HttpRequest req, StringBuffer page)
  {
    ...
    if (comps != null)
    {
      ...
      for (int i = 0, n = comps.length; i < n; i++)
      {
        ...
        page.append(comp.getID()).append("</td><td>").append(comp.getName())
            .append("</td><td>").append(
                formatServerTimeDate(comp.getStartTime())).append("</td><td>")
        ...
        page.append(" (<em>").append(comp.getStartUniqueID()).append(" - ")
            .append(comp.getEndUniqueID()).append("</em>)</td><td>").append(
                numAgents).append(" / ").append(numGames).append(
                "</td></tr>\r\n");
      }
      page.append("</table><p>\r\n");
    }
    ...
    page
        ...
        .append(
            "<table border='0'>\r\n"
                ...
                // + "<select name=type><option value=total>Total number of
                // games"
        ...
        .append(
            "'></td></tr>"
                ...
                // defaultGameLength + "' size=32></td></tr\r\n>"
    ...
    for (int i = 0, n = users.length; i < n; i++)
    {
      ...
      page.append("<td><input type=checkbox name=join-").append(
          users[i].getID()).append('>').append(users[i].getName()).append(
          "</td>");
    }
    if ((users.length % 5) > 0)
    {
  ...
  public static void main(String[] args)
  {
    ...
    //       System.out.println("LCD: 6 <?>" + i + " -> " + findLargestDivisor(6, i));
    //     }
