...
public class StatPageGenerator
{
      ...
      int end)
  {
    ...
    // Cut down unused areas (e.g. if newInterval is 20000 due to old was 10010
    ...
    for (int i = 0, n = len; i < n; i++)
    {
      ...
      if (i >= averageCount)
      {
        ...
        g2d.drawLine(xp - (int) ((avgPos + 1) * xResolution), oldMedP, xp
            - (int) (avgPos * xResolution), medP);
      }
      if (sc < min)
      {
        sc = min;
      ...
      CompetitionParticipant user, boolean generateImage)
  {
    ...
    try
    {
      ...
      {
        // Calculate the full participant score for the competition chain
        Competition parentCompetition = competition.getParentCompetition();
        // Create cache object for the user scores so we can change it freely
        user = new CompetitionParticipant(user);
        // Create a competition stack to get the latest games in correct order
        comps = new ArrayList();
        comps.add(competition);
        while (parentCompetition != null)
        {
          CompetitionParticipant u = parentCompetition.getParticipantByID(user
              .getID());
          if (u != null)
          {
            user.addScore(u);
      ...
      do
      {
        Competition currentCompetition = comps != null ? (Competition) comps
            .remove(comps.size() - 1) : competition;
        ...
        while (res.next())
        {
          ...
          if ((flags & SimServer.SIMULATION_SCRATCHED) == 0)
          {
            ...
            if ((agentScore == 0L) || (flags & SimServer.ZERO_GAME) != 0)
            {
      ...
      String title = "Statistics for " + user.getName() + " in competition "
          + competition.getName();
      ...
      if (generateImage)
      {
        ...
        else if (hasGUI)
        {
          ...
          out.tr().td(
              "<img src='" + user.getID() + "_gst.png' alt='Agent Scores'>");
          out.tableEnd();
      ...
      if (user.getZeroGamesPlayed() != zeroScoreCount)
      {
        log.log(Level.SEVERE, "Competition " + competition.getID()
            ...
            + user.getZeroGamesPlayed() + " zero games but " + " found "
            + zeroScoreCount + " zero games", new IllegalStateException(
            "mismatching zero games"));
      }
      ...
      {
        out.td(toString(user.getAvgScoreWithoutZeroGames()), "align=right");
      }
      ...
      if (lastNumberOfGames > 0)
      {
        ...
        for (int i = 0; i < lastNumberOfGames; i++)
        {
          ...
          if (gameIDs[i] > 0)
          {
            ...
            out.td("", "align=right").text("<a href='").text(urlGamePath).text(
                gameIDStr).text("/'>").text(gameIDStr).text("</a>");
          }
          else
          {
          ...
          out.td("", "align=right");
          if (((gameFlags[i] & SimServer.ZERO_GAME) != 0) && scores[i] < 0L)
          {
  ...
  private static HtmlWriter formatLong(HtmlWriter out, long score)
  {
    if (score < 0)
    {
      out.text("<font color=red>")
          .text(FormatUtils.formatLong(score, "&nbsp;")).text("</font>");
    }
    else
    {
  ...
  private static String toString(double score)
  {
    return score < 0 ? ("<font color=red>"
        + FormatUtils.formatDouble(score, "&nbsp;") + "</font>") : FormatUtils
        .formatDouble(score, "&nbsp;");
  }
