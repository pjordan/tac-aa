  ...
  public boolean createScoreTable(Competition competition, int gameID)
  {
    ...
    try
    {
      ...
      page.append("<html><head><title>TAC AA - Score Page for ").append(
          competition.getName()).append("</title></head>\r\n<body>\r\n");
      ...
      if (users != null)
      {
        ...
        if (isShowingCompetitionTimes)
        {
          ...
          page.append(
              InfoServer.getServerTimeAsString(competition.getEndTime()))
              .append(".</em><br>\r\n");
        }
        ...
        for (int i = 0, n = users.length; i < n; i++)
        {
          ...
          if (isShowingAllAgents
              || (isShowingZeroGameAgents ? (usr.getGamesPlayed() > 0) : (usr
                  .getGamesPlayed() > usr.getZeroGamesPlayed())))
          {
            ...
            page.append("<tr>").append(tdrank).append(pos++).append("</td>")
            ...
            page.append("</td>").append(tdright).append(usr.getGamesPlayed())
                .append("</td>");
            if (isShowingZeroGameAgents)
            {
              page.append(tdright).append(usr.getZeroGamesPlayed()).append(
                  "</td>");
            }
            page.append("</tr>\r\n");
          }
      ...
      int numberOfAgents)
  {
    ...
    if (isAddingStatisticsLink)
    {
      return "<a href='" + usr.getID() + ".html'>"
          + getAgentNameStyle(usr.getName(), pos, numberOfAgents) + "</a>";
    }
    return usr.getName();
  }
