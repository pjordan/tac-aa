...
public class AdminPage extends HttpPage
{
  ...
  private StringBuffer pageStart(String title)
  {
    ...
    page.append("<html><head><title>TAC AA - ").append(title).append('@')
        .append(serverName).append("</title></head>\r\n" + "<body>\r\n")
        .append(header).append("<font face='arial' size='+2'>").append(title)
        .append(" at ").append(serverName).append("</font><p>\r\n");
    return page;
  }
  ...
  private StringBuffer generateAdmin(HttpRequest req)
  {
    ...
    if (req.getParameter("generateResults") != null)
    {
      try
      {
        ...
        page.append("<hr><p>Result page generated for game ").append(gameID)
            .append("<p><hr><p>\r\n");
      }
      catch (Exception e)
      {
        ...
        page.append("<hr><p><font color=red>could not generate score page: ")
    ...
    else if (req.getParameter("generateCompetition") != null)
    {
      try
      {
        ...
        page.append("<hr><p>Result page generated for competition ").append(
            competitionID).append("<p><hr><p>\r\n");
      }
      catch (Exception e)
      {
        Logger.global.log(Level.WARNING,
        ...
        page.append(
            "<hr><p><font color=red>"
                + "could not generate competition results: ").append(e).append(
            "</font>").append("<p><hr><p>\r\n");
      }
    }
    else if (req.getParameter("scratchGame") != null)
    {
      ...
      {
        Logger.global
            .log(Level.WARNING, "AdminPage: could not scratch game", e);
        page.append("<hr><p><font color=red>" + "could not scratch game: ")
            .append(e).append("</font>").append("<p><hr><p>\r\n");
      }
    ...
    else if (req.getParameter("reserve") != null)
    {
      ...
      if (len != null && len.length() > 0 && tim != null && tim.length() > 0)
      {
        try
        {
          ...
          page.append("<hr><p>Requested time reservation starting at ").append(
              GameScheduler.formatServerTimeDate(startTime)).append(
              " lasting for ").append(length).append(" minutes.").append(
              "<p><hr><p>\r\n");
        }
        catch (Exception e)
        {
          Logger.global.log(Level.WARNING, "AdminPage: could not reserve time",
              e);
          page.append("<hr><p><font color=red>Could not reserve time: ")
    ...
    else if (req.getParameter("Force") != null)
    {
      ...
      if (w != null && w.length() > 0)
      {
        ...
        {
          Logger.global.log(Level.WARNING, "AdminPage: could not force weight",
              e);
          page.append("<hr><p><font color=red>Could not set forced weight: ")
              .append(e).append("</font><hr><p>\r\n");
        }
      }
    ...
    else if (req.getParameter("maxSchedule") != null)
    {
      ...
      if (m != null && m.length() > 0)
      {
        try
        {
          ...
          page.append("<hr><p>Agents are limited to be scheduled in " + max
              + " games in advanced<hr><p>");
        }
        catch (Exception e)
        {
          Logger.global.log(Level.WARNING,
          ...
          page.append(
              "<hr><p><font color=red>Could not limit the number "
                  + "of games an agent can schedule in advanced: ").append(e)
              .append("</font><hr><p>\r\n");
        }
      }
    ...
    if (SERVER_TIME)
    {
      ...
      page.append("<input type=submit name=timeToString value='Convert'>");
      if (timeMessage != null)
      {
    ...
    page.append(
        "<p><font face=arial size='+1'>Time Reservation</font><p>\r\n"
            ...
            + "starting at <input type=text name=reserveTime value='").append(
        GameScheduler.formatServerTimeDate(serverTime)).append(
    ...
    page
        .append("</textarea><br> <input type='submit' "
  ...
  private StringBuffer generateGames(HttpRequest req)
  {
    ...
    while (paramIterator.hasNext())
    {
      ...
      if (p.startsWith("remove"))
      {
        ...
        try
        {
          ...
          page.append("<b>Requested that game ").append(uniqSimID).append(
              " should be removed</b><p>\r\n");
          p = null;
        }
        catch (Exception e)
        {
        ...
        finally
        {
          if (p != null)
          {
            page.append("<font color=red>Could not remove game ").append(p)
                .append("</font><p>\r\n");
          }
        }
        break;
    ...
    page.append("<p>Current server time is ").append(
        ...
        "<p><form method=post>\r\n" + "<table border=1>\r\n"
            ...
            + "<th>Type</th>" + "<th>Participants</th>" + "<th>Status</th>"
            + "<th>&nbsp;</th>\r\n");
    if (simulations != null)
    {
      for (int i = 0, n = simulations.length; i < n; i++)
      {
        ...
        page.append(" (<em>").append(g.getID()).append("</em>)</td><td>")
            .append(InfoServer.getServerTimeAsString(g.getStartTime())).append(
                " (").append(minutes).append("&nbsp;min").append(
                seconds > 0 ? ("&nbsp;" + seconds + "&nbsp;sec") : "").append(
                ")</td><td>").append(
                simServer.getSimulationTypeName(g.getType())).append(
                "</td><td>");
        for (int j = 0, m = g.getParticipantCount(); j < m; j++)
        {
          ...
          page.append(simServer.getUserName(g, g.getParticipantID(j)));
        }
        page.append("&nbsp;").append("</td><td>").append(
            g.getStartTime() <= currentTime ? "Running" : "Coming").append(
            "</td><td>");
        if (g.hasSimulationID())
        {
          page.append("&nbsp;");
        ...
        {
          page.append("<input type=submit name='remove").append(g.getID())
              .append("' value=Remove>");
        }
        page.append("</td></tr>\r\n");
      }
  ...
  private StringBuffer generateCompetition(HttpRequest req)
  {
    ...
    if (req.getParameter("setLastFinished") != null)
    {
      try
      {
        ...
        page.append("<b>Only " + " competitions newer than competition id ")
    ...
    else if (req.getParameter("changeComp") != null)
    {
      ...
      try
      {
        ...
        page.append("<b>Competition ").append(name).append(" (").append(id)
            .append(") has been changed!");
      }
      catch (Exception e)
      {
        page.append("<font color=red><b>could not change competition: ")
            .append(e).append("</b></font><p>\r\n");
      }
        ...
        if (p.startsWith("remove"))
        {
          ...
          try
          {
            ...
            if (index >= 0)
            {
              ...
              competitions = simServer.getCompetitions();
              p = null;
              page.append("<b>Competition ").append(c.getName()).append(
                  " has been removed</b><p>\r\n");
            }
            else
            {
              p += ": not found";
    ...
    page.append("<form method=post>\r\n" + "<table border=1>\r\n"
        ...
        + "<th>End time</th>" + "<th>Game IDs</th>" + "<th>Agents/Games</th>"
        + "<th>&nbsp;</th>\r\n");
    ...
    else
    {
      for (int i = 0, n = competitions.length; i < n; i++)
      {
        ...
        if (historyPath != null)
        {
          page.append("<a href='").append(historyPath).append("competition/")
              .append(comp.getID()).append("/'>").append(comp.getID()).append(
                  "</a>");
        }
        else
        {
        ...
        page.append("</td><td>" + "<a href='?edit=").append(comp.getID())
            .append("'>").append(comp.getName()).append("</a></td><td>")
            .append(InfoServer.getServerTimeAsString(comp.getStartTime()))
            .append("</td><td>").append(
                InfoServer.getServerTimeAsString(comp.getEndTime())).append(
                "</td><td>");
        if (comp.hasSimulationID())
        {
          page.append(comp.getStartSimulationID()).append(" - ").append(
              comp.getEndSimulationID());
        }
        else
        {
          page.append("? - ?");
        ...
        page.append(" (<em>").append(comp.getStartUniqueID()).append(" - ")
            .append(comp.getEndUniqueID()).append("</em>)</td><td>").append(
                comp.getParticipantCount()).append(" / ").append(
                comp.getSimulationCount()).append(
                "</td><td><input type=submit name='remove")
            .append(comp.getID()).append("' value=Remove></td></tr>\r\n");
      }
    }
    page.append(
    ...
    if (editID != null)
    {
      try
      {
        ...
        page.append("<font face='arial' size='+1'>" + "Edit competition ")
            .append(competition.getName()).append(
                "</font>" + "<p>\r\n<form method=post>"
                    + "<input type=hidden name=compid value='").append(id)
            .append(
                "'>\r\n<p><table borde='0'>"
                    + "<tr><td>Competition name:</td><td>"
                    + "<input type=text size=32 name=compname value='").append(
        ...
        page.append(
            "'></td></tr></table>\r\n<p>" + "<font face='arial' size='+1'>"
                + "Agents in competition ").append(competition.getName())
            .append("</font>" + "<p>\r\n");
      ...
      {
        page.append("<p><font color=red>Could not view competition ").append(
            editID).append(": ").append(e).append("</font>");
      }
